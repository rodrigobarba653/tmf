...
import { whyTmflexContent, type WhyTmflexIconKey } from "@/content/site";

const iconMap: Record<WhyTmflexIconKey, LucideIcon> = {
  award: Award,
  globe: Globe,
  layers: Layers,
  wrench: Wrench,
    ...
    <section className="w-full max-w-8xl mx-auto px-4 sm:px-6 lg:px-16 lg:pt-8 pt-8 lg:pb-24 pb-16">
      <header className="mb-10 lg:mb-14">
        <p className="text-xs font-bold uppercase tracking-widest text-gray-500">
          {whyTmflexContent.subheading}
        </p>
        <h2 className="mt-2 text-3xl font-semibold text-foreground sm:text-4xl">
          {whyTmflexContent.title}
        </h2>
      </header>

      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
        {whyTmflexContent.reasons.map(({ text, icon }) => {
